...
#### MES 공정관리
- 용어
    - MES : Manufacturing Execution 생산실행관리 시스템
        - 생산현장에서 실시간으로 제조/생산 작업, 계획, 실행, 추적, 모니터링하는 시스템
        - 작업지시, 생산실적, `품질관리`, `설비가동모니터링` 등
        - ERP(회사의 모든 데이터)에서 제조/생산에 관련된 데이터를 전달받아서 실시간 처리한뒤 결과를 다시 ETP로 전달
    - MRP : Material Requirment Planning 자재 소요 계획
...
- IoT 디바이스는 C# 시뮬레이터로 대체, MQTT Publish 병행
...
- [3D센서](https://www.devicemart.co.kr/goods/view?no=14930970) : 부품 조립상태, 오차, 두계불량
...
#### 파이썬 AI + ASP.NET 연동
- 참조소스 
- iot-webapp-2025/ref/ReferencedSolution/CovertWebapp
